...
스크립트를 실행하여 Hook/유틸리티 연동, 커맨드-에이전트 참조, 에이전트 테이블, 버전 정합성을 자동 점검한다.
...
- [F2] Check 1~4 중 FAIL 항목이 있으면 상세 내용을 감사 결과에 포함

> 스크립트 출력을 그대로 감사 결과에 첨부한다. FAIL이 있으면 NG로 표시한다.
...
| 항목 | 점검 내용 | 결과 | 상세 |
|------|---------|------|------|
| F1 | integrity-check 전체 PASS | {OK/NG} | {상세} |
| F2 | FAIL 상세 (있는 경우) | {OK/NG} | {상세} |
