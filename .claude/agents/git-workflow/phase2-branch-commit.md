...
```
{type}: {brief description} (#ITSM-{number})
...
예시:
```
fix: 세션 만료 후 자동 로그아웃 처리 (#ITSM-3421)

근본 원인: 토큰 갱신 실패 시 예외가 무시되어 만료된 세션이
유지됨. 이후 API 호출에서 401 오류가 연속 발생.

수정: 토큰 갱신 실패 시 즉시 로그아웃 처리 후 로그인 페이지로
리다이렉트. AuthInterceptor에서 refresh 실패 케이스 추가.
```
...
# 3. Commit — body가 항상 있으므로 항상 HEREDOC 사용
...
**Multi-line message rule:** body가 있는 커밋은 항상 HEREDOC(`$(cat <<'EOF' ... EOF)`)을 사용한다. `-m "..."` 직접 전달 시 줄바꿈이 유지되지 않아 body가 소실된다.
