...
```bash
# v0.0.1 이후 전체 커밋 (커밋 컨벤션, 브랜치 전략 점검용)
git log v0.0.1..HEAD --oneline

# v0.0.3 이후 커밋 (ITSM 번호 점검용)
git log v0.0.3..HEAD --oneline
```

### 2단계: 커밋 컨벤션 준수 점검 (v0.0.1 이후)

v0.0.1 이후의 커밋 메시지를 검사한다.

```bash
# v0.0.1 이후 머지 커밋 제외한 일반 커밋
git log v0.0.1..HEAD --oneline --no-merges
```

점검 항목:
- [A1] 모든 커밋이 "type(scope): 설명" 형식을 따르는가
  - 허용 type: feat, fix, improve, docs, refactor, chore
  - scope: EQP_ID (예: PRS-04)
  - "merge:" 접두사는 머지 커밋이므로 이 점검에서 제외
- [A2] "chore: bump version" 외에 type이 누락된 커밋이 없는가

### 3단계: ITSM 번호 연결 점검 (v0.0.3 이후)

v0.0.3 이후의 커밋을 검사한다.

```bash
# v0.0.3 이후 대표 커밋 (feat, fix, improve, docs, refactor)
git log v0.0.3..HEAD --oneline --no-merges --grep="^feat\|^fix\|^improve\|^docs\|^refactor"

# v0.0.3 이후 머지 커밋
git log v0.0.3..HEAD --oneline --merges
```
...
### 4단계: 브랜치 전략 준수 점검 (v0.0.1 이후)

```bash
# v0.0.1 이후 master 위의 직접 커밋 확인 (머지 커밋 제외)
git log v0.0.1..HEAD --oneline --no-merges --first-parent master
```
...
```bash
# 머지 커밋 확인
git log v0.0.1..HEAD --oneline --merges --first-parent master
```
...
### 1. 커밋 컨벤션 준수 (v0.0.1 이후)

| 항목 | 점검 내용 | 결과 | 상세 |
|------|---------|------|------|
| A1 | type(scope): 설명 형식 준수 | {OK/NG} | {상세} |
| A2 | type 누락 커밋 없음 | {OK/NG} | {상세} |

### 2. ITSM 번호 연결 (v0.0.3 이후)
