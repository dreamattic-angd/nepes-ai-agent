...
readStdinJson((err, input) => {
  if (err) {
    console.error('[check-sensitive-write] 입력 파싱 실패 — 검증을 건너뜁니다:', err.message);
    process.exit(0);
    return;
  }
  const filePath = (input.tool_input && (input.tool_input.file_path || input.tool_input.path || ''));
  // path.resolve로 절대경로화 → ../ 경로 우회(path traversal) 방어
  const resolved = filePath ? path.resolve(filePath.replace(/\\/g, '/')) : '';
