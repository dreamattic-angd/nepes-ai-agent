...
| ai-guideline-analyzer | ✅ 운영 | AI 지침 파일 품질 자동 분석 (Static+Adaptive Rubric, 100점 만점) | Phase 기반 (5단계) | `/ai-guideline-analyzer` |
...
Claude Code 프로젝트 내 AI 지침 파일(agent/command/hook/skill/CLAUDE.md/eval/config 등)의 품질을 자동 분석합니다.

### 주요 기능
- 파일 내용 기반 유형 자동 분류 (경로 패턴 1차 → 내용 분석 2차)
- Static Rubric S1-S8 (80점): 역할 정의·트리거·출력 형식·검증 체크리스트·개입 지점·불확실 처리·길이·구체성
- Adaptive Rubric (20점): 파일 유형별 3-5개 항목 동적 생성 (agent/command/hook/skill/eval/config/CLAUDE.md 등)
- 5개 이상 파일 시 병렬 Sub-agent 처리, 결과를 `guideline-analysis-report.md`로 저장

### 워크플로우 (Phase 기반)
```
Phase 0: Glob 탐색 + 비AI 파일 제외
Phase 1: 파일 유형 분류 + 분석 방식 결정 (순차 / 병렬)
Phase 2: Static Rubric(S1-S8) + Adaptive Rubric 평가
Phase 3: 전체 요약 (Top/Bottom 3)
Phase 4: guideline-analysis-report.md 저장
```

### 상세 문서
→ `.claude/agents/ai-guideline-analyzer.md`, `.claude/commands/ai-guideline-analyzer.md`
