...
| language-router | ✅ 운영 | 변경 파일 언어 감지 후 적절한 code-reviewer로 라우팅 (Python/Frontend/Java/JS 구분) | 단일 | code-review Step 1.5 자동 호출 |
...
| frontend-code-reviewer | ✅ 운영 | React/Next.js/a11y/UI/UX 특화 코드 리뷰 (Server Component, 접근성, 번들 최적화) | 단일 | language-router 자동 호출 |
| python-code-reviewer | ✅ 운영 | Python/FastAPI/Pydantic/async 특화 코드 리뷰 (이벤트루프 차단, DB 연결 관리 등) | 단일 | language-router 자동 호출 |
| developer-frontend | ✅ 운영 | Next.js/React/TypeScript 구현 전용 서브에이전트 (Server/Client Component 분리, CSS Modules) | 단일 | workflow-automate 자동 호출 또는 직접 호출 |
| developer-backend-java | ✅ 운영 | Java/Spring Boot 구현 전용 서브에이전트 (레이어드 아키텍처, @Transactional, JPA 안전성) | 단일 | workflow-automate 자동 호출 또는 직접 호출 |
| developer-backend-python | ✅ 운영 | Python/FastAPI 구현 전용 서브에이전트 (Pydantic, async, numpy/scipy threadpool 패턴) | 단일 | workflow-automate 자동 호출 또는 직접 호출 |
...
**구조 특징**: Phase 파일 대신 모드별 가이드 파일 (`review-full.md`, `review-quick.md`) + 언어별 라우터 (`language-router.md` → `java-code-reviewer.md` / `js-code-reviewer.md` / `frontend-code-reviewer.md` / `python-code-reviewer.md`)
