...
### 특징
- **브랜치 전략**: Trunk-Based (main/master → feature)
...
**배포 대상 (.claude/ → ~/.claude/):**
- `commands/`, `agents/`, `hooks/`, `scripts/`, `log-analyzer/`, `skills/` → xcopy 덮어쓰기
- `settings.json` → 보존형 덮어쓰기 (개인 설정 보존)
- `.mcp.json` → 배포 서버 덮어쓰기 + 사용자 MCP 서버 보존
- `CLAUDE.md` → 프로젝트 루트에서 글로벌로 배포 (동일 파일 공유)
...
├── install-merge.js                 ← JSON 보존형 덮어쓰기 merge
└── .claude/                         ← 개발환경 겸 배포 소스 (단일 소스)
    ├── README.txt                   ← 이 파일 (프로젝트 개요)
    ├── version.txt                  ← 버전 관리
    ├── CLAUDE.local.md              ← 개인/프로젝트별 설정 (git 미추적, 자동 로드)
    ...
    │   ├── analyze-secsgem.md
    │   ├── cm-audit.md              ← 형상 감사 (통합, 프로젝트 매핑)
    │   ├── code-review.md
    │   ├── issue-analyze.md
    │   ├── ppqa-audit.md            ← PPQA 셀프 감사 (통합, 프로젝트 매핑)
    │   ├── pr-review.md
    │   ├── weekly-report.md         ← 주간 보고 (통합, 프로젝트 매핑)
    │   ├── handoff.md               ← 작업 인수인계 문서 생성
    │   ├── review-claudemd.md       ← CLAUDE.md 건강 점검
    │   └── git-workflow.md          ← 전체 프로젝트 공용 Git 워크플로우 (자동 감지, NAA 포함)
    ├── agents/                      ← 에이전트 구현
    │   ├── secsgem-analysis/
    │   │   ├── README.txt
    ...
    │   │   ├── ppqa-audit/
    │   │   └── weekly-report/
    │   └── git-workflow/             ← 전체 프로젝트 공용 Git 워크플로우 (NAA 포함)
