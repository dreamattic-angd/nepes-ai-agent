...
| 단계 | 소스 | 대상 | 동작 |
|------|------|------|------|
| **0/3** 백업 | `~/.claude/` 기존 파일 | `~/.claude/_backup/` | 직전 1세대 백업 (덮어쓰기 전 안전망) |
| **1/3** 전체 배포 | `.claude/` 전체 | `~/.claude/` | robocopy 전체 복사 (`settings.json`, `settings.local.json` 제외) |
| **2/3** 설정 병합 | `.claude/settings.json`, `.claude/.mcp.json`, `CLAUDE.md` | `~/.claude/` | 보존형 병합 (아래 상세) |
| **3/3** 배포 검증 | `~/.claude/` | — | `install-verify.js`로 서브디렉토리 배포 확인 |
...
`.claude/` 디렉토리를 통째로 대상에 복사합니다 (`robocopy /E`). `settings.json`, `settings.local.json`은 제외됩니다 (병합 단계에서 별도 처리).

| 디렉토리 | 내용물 | 용도 |
|----------|--------|------|
| `commands/` | 슬래시 커맨드 정의 (`.md`) | `/{command}`로 에이전트 실행 |
| `agents/` | 에이전트 구현 (Phase별 `.md`) | 커맨드에서 참조하는 실행 로직 |
| `hooks/` | PreToolUse/PostToolUse Hook (`.js`) | DB write 차단, 위험 명령어 차단, CRLF 자동 변환 등 |
| `scripts/` | 유틸리티 스크립트 (`.js`) | 상태 라인, 무결성 검증, 워크플로우 통계, Eval 등 |
| `evals/` | Eval 골든 셋 (`.json`) | 에이전트 품질 자동 검증 |
| `skills/` | Skill 구현 | 자연어 트리거 기반 기능 |
| `log-analyzer/` | 로그 분석 도구 | FTP 로그 수집 스크립트 + 설비 정보 캐시 |

#### 2단계: 보존형 병합 (`install-merge.js`)
...
| `/pr-review` | PR 코드 리뷰 |
| `/cm-audit` | 형상 감사 |
| `/ppqa-audit` | PPQA 셀프 감사 |
| `/handoff` | 작업 인수인계 문서 생성 |
| `/review-claudemd` | CLAUDE.md 건강 점검 |
| `/monitoring-report` | 하네스 모니터링 리포트 |
| `/distill-failures` | 실패 패턴 증류 |
| `/help-cmd` | 커맨드 도움말 |
| `/git-workflow` | Git 워크플로우 (전체 프로젝트 자동 감지) |
